...
impl std::str::FromStr for OAuthChallengeKey {
    type Err = nova_framework::Error;
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let hex_bytes = hex::decode(s).map_err(|_| nova_framework::Error::InvalidInput)?;
        let array = hex_bytes
            .try_into()
            .map_err(|_| nova_framework::Error::InvalidInput)?;
        Ok(Self(array))
    }
}
...
impl redis::ToRedisArgs for OAuthChallengeKey {
        ...
        W: ?Sized + redis::RedisWrite,
    {
        ...
        let key: nova_framework::redis::RedisKey = nova_framework::redis::RedisKey::from(string);
...
impl nova_framework::redis::KeyValue for OAuthChallenge {
...
impl nova_framework::redis::KeyValueRead for OAuthChallenge {}
impl nova_framework::redis::KeyValueWrite for OAuthChallenge {}
...
impl OAuthChallenge {
    ...
    pub async fn read_and_delete(
        conn: &mut nova_framework::redis::RedisConnection,
        key: OAuthChallengeKey,
    ) -> Result<Option<Self>, nova_framework::Error> {
        ...
        if let Some(bytes) = data {
            let val = <Self as MessageDe>::from_bytes(&bytes)
                .map_err(|e| nova_framework::Error::DeserializeError(e.into()))?;
            Ok(Some(val))
        } else {
            Ok(None)
        }
